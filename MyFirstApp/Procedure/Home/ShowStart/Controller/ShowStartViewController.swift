...
class ShowStartViewController: XYBaseViewController {
    ...
    open var city: ShowStartCityDataModel? {
        didSet {
            let rightButton = ShowStartCityButton(frame: CGRect(x: 0, y: 0, width: 30, height: 15))
            
            rightButton.textLabel.text = city?.cityName
    ...
    private lazy var searchButton =  ShowStartSearchBarButton(type: .system).then {
