...
extension ShowStartCityViewModel {
    func refreshDataSource()  {
        ...
        ShowStartAPIProvider.request(.cityList) { [weak self](result) in
            if case let .success(response) = result {
                if let data = try? response.mapJSON(),let mappedObject = JSONDeserializer<ShowStartResponseData<[ShowStartCityResultModel]>>.deserializeFrom(json: JSON(data).description){
